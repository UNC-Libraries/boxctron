...
class TestImageAugmentor:
  ...
  def test_process(self, config, tmp_path):
    ...
    assert len(aug_annos) == 15 # 14 original + 1 augmented
    assert aug_annos[14]['image'] == str(output_path)
  ...
  def test_label_augmentation(self, config, tmp_path):
    ...
    assert len(aug_annos) == 15 # 14 original + 1 augmented
    assert aug_annos[14]['image'] == str(output_path)
    assert aug_annos[14]['rotation_type'] == 'r90'
    assert len(aug_annos[14]['label']) == 2 #Subject, color_bar
    assert aug_annos[14]['label'][0]['x'] == 0 
    assert aug_annos[14]['label'][0]['y'] == 0
    assert aug_annos[14]['label'][0]['width'] == 17.7316
    assert aug_annos[14]['label'][0]['height'] == 98.9297
    assert aug_annos[14]['label'][0]['original_width'] == 1276
    assert aug_annos[14]['label'][0]['original_height'] == 1333
  ...
  def test_augmentation_no_labels(self, config, tmp_path):
    ...
    assert len(aug_annos) == 15 # 14 original + 1 augmented
    assert aug_annos[14]['image'] == str(output_path)
    assert aug_annos[14]['rotation_type'] == 'rfh'
    assert aug_annos[14].get('label') == None
  ...
  def test_process_with_rotation_and_saturation(self, config, tmp_path):
    ...
    assert aug_annos[14]['annotation_id'] == 8
    assert aug_annos[14]['rotation_type'] == 'r90'
    assert aug_annos[14]['label'][0]['original_width'] == 1076 # Swap height and width from annotation
    assert aug_annos[14]['label'][0]['original_height'] == 1333
    assert sum(1 for x in config.output_base_path.rglob('*') if x.is_file()) == 1
  ...
  def test_process_image_twice(self, config, tmp_path):
    ...
    aug_annos = from_json(config.annotations_output_path)
    assert len(aug_annos) == 16 # 14 original + 2 augmented
    assert aug_annos[14]['image'] == str(output_path)
    assert aug_annos[14]['annotation_id'] == 8
    assert aug_annos[14]['rotation_type'] == 'r90'
    assert aug_annos[15]['image'] == str(output_path2)
    assert aug_annos[15]['annotation_id'] == 8
    assert aug_annos[15]['rotation_type'] == 'r90fh'
